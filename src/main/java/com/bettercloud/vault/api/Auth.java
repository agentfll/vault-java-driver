...
public class Auth {
    ...
    /**
     * A convenience method for invoking loginByAppRole with the default path.
    ...
    /**
     * <p>Authenticate with Vault using the AppRole backend.  Example usage:</p>
     *
     * <blockquote>
     * <pre>{@code
     * final AuthResponse response = vault.auth().loginByAppRole("approle", "role_id", "secret_id");
     *
     * final String token = response.getAuthClientToken();
     * }</pre>
     * </blockquote>
     *
     * @param path The path on which the authentication is performed (e.g. <code>approle</code>)
     * @param roleId The role ID used for authentication
     * @param secretId The secret ID used for authentication
     * @return The auth token
     * @throws VaultException If any error occurs, or unexpected response received from Vault
